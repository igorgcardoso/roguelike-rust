...
pub fn town_builder(
    ...
    let mut chain = BuilderChain::new(new_depth, width, height, "Naven");
...
impl TownBuilder {
    ...
    pub fn build_rooms(
        ...
        for y in wall_gap_y - 3..wall_gap_y + 4 {
            let idx = build_data.map.xy_idx(build_data.width - 2, y);
    ...
    fn town_walls(
        ...
        for y in 1..build_data.map.height - 2 {
            if !(y > wall_gap_y - 4 && y < wall_gap_y + 4) {
                ...
                for x in 30..build_data.map.width - 2 {
        ...
        for x in 30..build_data.width - 1 {
            ...
            let idx_bottom = build_data.map.xy_idx(x, build_data.height - 2);
    ...
    fn buildings(
            ...
            for y in by..by + bh {
                for x in bx..bx + bw {
                        ...
                        || y > build_data.map.height - 1
                    {
                        ...
                        let idx = build_data.map.xy_idx(x, y);
                        if !available_buildings_tiles.contains(&idx) {
                            possible = false;
                        }
                    }
                }
            ...
            if possible {
                ...
                for y in by..by + bh {
                    for x in bx..bx + bw {
                        let idx = build_data.map.xy_idx(x, y);
                        build_data.map.tiles[idx] = TileType::WoodFloor;
                        available_buildings_tiles.remove(&idx);
                        available_buildings_tiles.remove(&(idx + 1));
                        available_buildings_tiles.remove(&(idx + build_data.map.width as usize));
                        available_buildings_tiles.remove(&(idx - 1));
                        available_buildings_tiles.remove(&(idx - build_data.map.width as usize));
                    }
                }
        ...
        // Outline buildings
        let mut map_clone = build_data.map.clone();
        for y in 2..build_data.height - 2 {
            for x in 32..build_data.width - 2 {
                ...
                if build_data.map.tiles[idx] == TileType::WoodFloor {
                    ...
                    if neighbors > 0 {
                        map_clone.tiles[idx] = TileType::Wall;
                    }
                }
            }
        }
        build_data.map = map_clone;
        build_data.take_snapshot();
        ...
        for door_idx in doors.iter() {
                ...
                nearest_roads.push((
                    *road,
                    rltk::DistanceAlg::Pythagoras.distance2d(
                        door_point,
                        rltk::Point::new(
                            *road as i32 % build_data.map.width,
                            *road as i32 / build_data.map.width,
                        ),
                    ),
                ));
            ...
            if path.success {
                for step in path.steps.iter() {
                    build_data.map.tiles[*step] = TileType::Road;
                    roads.push(*step);
                }
            }
            build_data.take_snapshot();
    ...
    fn sort_buildings(
        ...
        for building in building_size.iter_mut().skip(6) {
            building.2 = BuildingTag::Hovel;
        }
        ...
        for y in building.1..building.1 + building.3 {
            for x in building.0..building.0 + building.2 {
                ...
                if build_data.map.tiles[idx] == TileType::WoodFloor
                    ...
                    && !to_place.is_empty()
                {
                    let entry_tag = to_place[0];
                    to_place.remove(0);
                    build_data.spawn_list.push((idx, entry_tag.to_string()));
                }
            }
        }
    }
