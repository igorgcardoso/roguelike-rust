...
pub fn render_pattern_to_map(
    ...
    for (x, north_bound) in chunk.exits[0].iter().enumerate() {
        if *north_bound {
    ...
    for (x, south_bound) in chunk.exits[1].iter().enumerate() {
        if *south_bound {
    ...
    for (y, west_bound) in chunk.exits[2].iter().enumerate() {
        if *west_bound {
    ...
    for (y, east_bound) in chunk.exits[3].iter().enumerate() {
        if *east_bound {
