...
impl Solver {
    ...
    pub fn iteration(&mut self, map: &mut Map, rng: &mut super::RandomNumberGenerator) -> bool {
        ...
        // Populates the neighbor count of the remaining list
        let mut remain_copy = self.remaining.clone();
        let mut neighbors_exists = false;
        for remain in remain_copy.iter_mut() {
            let idx = remain.0;
            let chunk_x = idx % self.chunks_x;
            let chunk_y = idx / self.chunks_x;
            let neighbors_count = self.count_neighbors(chunk_x, chunk_y);
            if neighbors_count > 0 {
                neighbors_exists = true;
            }
        ...
        let remaining_index = if !neighbors_exists {
