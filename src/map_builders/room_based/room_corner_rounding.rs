...
impl RoomCornerRounder {
    ...
    fn fill_if_corner(&mut self, x: i32, y: i32, build_data: &mut BuilderMap) {
        ...
        if y > 0 && build_data.map.tiles[idx - width as usize] == TileType::Wall {
            neighbor_walls += 1;
        }
        if x < width - 2 && build_data.map.tiles[idx + 1] == TileType::Wall {
            neighbor_walls += 1;
        }
        if y < height - 2 && build_data.map.tiles[idx + width as usize] == TileType::Wall {
            neighbor_walls += 1;
        }
    ...
    fn build(&mut self, _rng: &mut RandomNumberGenerator, build_data: &mut BuilderMap) {
        ...
        if let Some(rooms_builder) = &build_data.rooms {
            ...
            panic!("Room Corner Rounder requires a builder with room structures");
        }
