...
impl RoomDrawer {
    ...
    fn circle(&mut self, build_data: &mut BuilderMap, room: &Rect) {
        ...
        let center_point = rltk::Point::new(center.0, center.1);
        for y in room.y1..=room.y2 {
            for x in room.x1..=room.x2 {
                let idx = build_data.map.xy_idx(x, y);
                let distance =
                    rltk::DistanceAlg::Pythagoras.distance2d(center_point, rltk::Point::new(x, y));
                if idx > 0
    ...
    fn build(&mut self, rng: &mut RandomNumberGenerator, build_data: &mut BuilderMap) {
        ...
        if let Some(rooms_builder) = &build_data.rooms {
            ...
            panic!("RoomDrawer requires a builder with room structures");
        }
