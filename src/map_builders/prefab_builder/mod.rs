...
use super::{
    remove_unreachable_areas_returning_most_distant, Map, MapBuilder, Position, TileType,
...
impl PrefabBuilder {
    ...
    pub fn rex_level(new_depth: i32, template: &'static str) -> PrefabBuilder {
        let mut builder = PrefabBuilder::new(new_depth, None);
        builder.mode = PrefabMode::RexLevel { template };
        builder
    }
    ...
    pub fn constant(new_depth: i32, level: prefab_levels::PrefabLevel) -> PrefabBuilder {
        let mut builder = PrefabBuilder::new(new_depth, None);
        builder.mode = PrefabMode::Constant { level };
        builder
    }
    ...
    pub fn sectional(
        ...
        let mut builder = PrefabBuilder::new(new_depth, Some(previous_builder));
        builder.mode = PrefabMode::Sectional { section };
        builder
    }
    ...
    pub fn vaults(new_depth: i32, previous_builder: Box<dyn MapBuilder>) -> PrefabBuilder {
        let mut builder = PrefabBuilder::new(new_depth, Some(previous_builder));
        builder.mode = PrefabMode::RoomVaults;
        builder
    }
    ...
    fn apply_room_vaults(&mut self) {
        ...
        if possible_vaults.is_empty() {
            return; // Bail out if there's nothing to build
        }
